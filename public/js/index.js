...
function setScales(time, type, Data){
  ...
  if(type === 'soilMoisture'){ // If type is soilMoisture limit to between 0% and 100%
    ...
    // padding can range from +/- 20 to +/- 30 AND limited from 0 to 50
    y = d3.extent(Data, d => d.desired).map((x, i, a) => i ? Math.min(50, x+20/(a[1]-a[0] > 0.667 ? a[1] - a[0] : 1)) : Math.max(0, x-20/(a[1]-a[0] > 0.667 ? a[1] - a[0] : 1)));
  } else {
